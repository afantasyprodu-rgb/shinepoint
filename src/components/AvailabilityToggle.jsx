  ...
  if (!current) {
    return (
      <section className="card" aria-busy={!error}>
        <h2 className="font-display text-lg font-semibold text-slate-900">Availability</h2>
        {error ? (
          <p role="alert" className="mt-2 text-sm text-red-700">{error}</p>
        ) : (
          <div className="mt-3 h-12 animate-pulse rounded-xl bg-brand-100" />
        )}
      </section>
    )
  }
  ...
  return (
    <section className="card">
      ...
      <div
        ...
        className="mt-3 grid grid-cols-3 gap-1 rounded-xl bg-brand-50 p-1"
      >
        {OPTIONS.map(({ value, label, active }) => (
          <button
            ...
            className={`h-11 cursor-pointer rounded-lg text-sm font-semibold transition-all duration-200 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-600 ${
              current.status === value ? active : 'text-slate-600 hover:bg-brand-100'
            }`}
          >
            {label}
          </button>
