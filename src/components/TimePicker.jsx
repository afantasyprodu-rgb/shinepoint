...
export default function TimePicker({ value, onChange }) {
  ...
  return (
    <div className="rounded-3xl bg-[#F3EFFA] p-5" role="group" aria-label="Pick a time">
      <div className="flex flex-col items-center gap-3">
        <div className="relative h-36 w-36 shrink-0 rounded-full bg-[#F3EFFA] shadow-[8px_8px_16px_rgba(124,58,237,0.14),-8px_-8px_15px_rgba(255,255,255,0.9)]">
          {Array.from({ length: 12 }).map((_, i) => (
            <div key={i} className="absolute inset-0" style={{ transform: `rotate(${i * 30}deg)` }}>
              <span className="absolute left-1/2 top-2 h-2.5 w-[3px] -translate-x-1/2 rounded-full bg-brand-300/70" />
            </div>
          ))}

          <div className="absolute inset-0" style={{ transform: `rotate(${hourDeg}deg)`, transition: handTransition }}>
            <span className="absolute left-1/2 w-[5px] -translate-x-1/2 rounded-full bg-brand-800" style={{ top: 'calc(50% - 30px)', height: 30 }} />
          </div>
          <div className="absolute inset-0" style={{ transform: `rotate(${minuteDeg}deg)`, transition: handTransition }}>
            <span className="absolute left-1/2 w-[3.5px] -translate-x-1/2 rounded-full bg-brand-600" style={{ top: 'calc(50% - 48px)', height: 48 }} />
          </div>

          <span className="absolute left-1/2 top-1/2 h-2.5 w-2.5 -translate-x-1/2 -translate-y-1/2 rounded-full bg-brand-800 shadow-[0_1px_2px_rgba(0,0,0,.35)]" />
        </div>

        <p className="font-display text-base font-bold tabular-nums text-brand-900">
            ...
            <button
              ...
              className={`flex shrink-0 flex-col items-center rounded-2xl px-3.5 py-2.5 transition-all duration-300 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-600 focus-visible:ring-offset-2 ${
                active
                  ...
                  : 'bg-[#F3EFFA] text-brand-900 shadow-[6px_6px_12px_rgba(124,58,237,0.13),-6px_-6px_11px_rgba(255,255,255,0.9)] hover:shadow-[4px_4px_9px_rgba(124,58,237,0.16),-4px_-4px_8px_rgba(255,255,255,0.95)]'
              }`}
            >
              <span className="font-display text-sm font-semibold tabular-nums">{t.text}</span>
              <span className={`text-[10px] font-semibold uppercase tracking-wider ${active ? 'text-white/80' : 'text-brand-500'}`}>{t.ampm}</span>
            </button>
          )
        })}
      </div>
